...
from ezpz.dist import get_rank, get_world_size, setup_wandb  # , setup_torch

RANK = get_rank()
WORLD_SIZE = get_world_size()

if RANK == 0 and not (
    (wbmode := os.environ.get('WANDB_MODE'))
    and (wbmode is not None and str(wbmode).lower() == 'disabled')
):
    project_name = (
        os.environ.get(
            'WB_PROJECT',
